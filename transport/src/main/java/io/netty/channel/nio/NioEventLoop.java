...
public final class NioEventLoop extends SingleThreadEventLoop {
  ...
  /**
   * Boolean that controls determines if a blocked Selector.select should break out of its selection
   * process. In our case we use a timeout for the select method and the select method will block
   * for that time unless waken up.
   */
  ...
  /**
   * Sets the percentage of the desired amount of time spent for I/O in the event loop.  The default
   * value is {@code 50}, which means the event loop will try to spend the same amount of time for
  ...
  private void rebuildSelector0() {
    ...
    for (SelectionKey key : oldSelector.keys()) {
      Object a = key.attachment();//----->可以在SelectionKey上关联一个对象，而且仅能关联一个，将来可以使用这个接口获取关联的对象
      try {
        //要求key有效，且key对应的channel没有注册到新的selector上
        ...
        //关闭旧的key
        key.cancel();
        //注册channel到新的selector上，并产生新的key
  ...
  private void processSelectedKeys() {
    //不为空，意味着使用了优化后的selector
    if (selectedKeys != null) {
      //不使用jdk的select方法，性能更好（提升1%-2%），垃圾回收更少
