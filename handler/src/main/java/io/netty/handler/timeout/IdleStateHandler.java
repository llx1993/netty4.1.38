...
/**
 * Triggers an {@link IdleStateEvent} when a {@link Channel} has not performed read, write, or both
 ...
 * will be triggered when no read was performed for the specified period of time.  Specify {@code 0}
 * to disable.</td>
 * </tr>
 * <tr>
 * <td>{@code writerIdleTime}</td>
 * <td>an {@link IdleStateEvent} whose state is {@link IdleState#WRITER_IDLE}
 * will be triggered when no write was performed for the specified period of time.  Specify {@code
 * 0} to disable.</td>
 * </tr>
 * <tr>
...
public class IdleStateHandler extends ChannelDuplexHandler {
   ...
   *
   * @param readerIdleTimeSeconds an {@link IdleStateEvent} whose state is {@link
   * IdleState#READER_IDLE} will be triggered when no read was performed for the specified period of
   * time.  Specify {@code 0} to disable.
   * @param writerIdleTimeSeconds an {@link IdleStateEvent} whose state is {@link
   * IdleState#WRITER_IDLE} will be triggered when no write was performed for the specified period
   * of time.  Specify {@code 0} to disable.
   ...
   * Creates a new instance firing {@link IdleStateEvent}s.
   *
   * @param observeOutput 是否观察 ChannelOutboundBuffer 写入队列 whether or not the consumption of {@code
   * bytes} should be taken into consideration when assessing write idleness. The default is {@code
   * false}.
   * @param readerIdleTime an {@link IdleStateEvent} whose state is {@link IdleState#READER_IDLE}
   * will be triggered when no read was performed for the specified period of time.  Specify {@code
   * 0} to disable.
   ...
   * triggered when neither read nor write was performed for the specified period of time.  Specify
   * {@code 0} to disable.
   * @param unit the {@link TimeUnit} of {@code readerIdleTime}, {@code writeIdleTime}, and {@code
   * allIdleTime}
   */
  ...
  /**
   * Is called when an {@link IdleStateEvent} should be fired. This implementation calls {@link
  ...
  protected IdleStateEvent newIdleStateEvent(IdleState state, boolean first) {
    switch (state) {
      case ALL_IDLE:
        return first ? IdleStateEvent.FIRST_ALL_IDLE_STATE_EVENT
            : IdleStateEvent.ALL_IDLE_STATE_EVENT;
      case READER_IDLE:
        return first ? IdleStateEvent.FIRST_READER_IDLE_STATE_EVENT
            : IdleStateEvent.READER_IDLE_STATE_EVENT;
      case WRITER_IDLE:
        return first ? IdleStateEvent.FIRST_WRITER_IDLE_STATE_EVENT
  ...
  /**
   * Returns {@code true} if and only if the {@link IdleStateHandler} was constructed with {@link
   * #observeOutput} enabled and there has been an observed change in the {@link
